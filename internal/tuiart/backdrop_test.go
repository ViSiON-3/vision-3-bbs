...
// cellAt exposes a composited cell for assertions.
func (b *Backdrop) cellAt(row, col int) Cell { return b.cells[row][col] }
...
func TestPickBackdropArt_ReturnsEmbedded(t *testing.T) {
	...
	if !found {
		t.Fatal("Pick returned bytes not in the embedded set")
	}
}
...
func TestLoadBackdropFrom_StripsSAUCE(t *testing.T) {
	...
	b := LoadFrom(raw, 80, 25)
	if !b.HasArt() {
		t.Fatal("expected art backdrop after SAUCE strip")
	}
	if b.cellAt(0, 0).ch != 'h' || b.cellAt(0, 1).ch != 'i' {
		t.Fatalf("art content lost: got %q%q", b.cellAt(0, 0).ch, b.cellAt(0, 1).ch)
	}
}

func TestRasterizeArt_Dimensions(t *testing.T) {
	grid := Rasterize([]byte("hello"))
...
func TestRasterizeArt_SGRColors(t *testing.T) {
	...
	grid := Rasterize([]byte("\x1b[1;33;44mX"))
	c := grid[0][0]
	if c.ch != 'X' {
...
func TestLoadBackdrop_OddMargin(t *testing.T) {
	b := Load(81, 25) // (81-80)/2 = 0 → art starts at col 0
