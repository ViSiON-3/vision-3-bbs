...
type newscanJoinPlan struct {
	...
	SeenTags     []string            // tags to mark seen regardless of prompt outcome
	SeenNets     []string            // lowercase networks to mark seen regardless of outcome
}
...
func buildNewscanJoinPlan(areas []*message.MessageArea, u *user.User, canRead func(*message.MessageArea) bool) newscanJoinPlan {
	...
	for _, area := range areas {
		...
		if area.Network == "" {
			...
			plan.SeenTags = append(plan.SeenTags, area.Tag)
			continue
		}
		...
		if !promptedNets[net] {
			...
			plan.SeenNets = append(plan.SeenNets, net)
		}
	}
	return plan
}
...
// genuinely new areas/networks trigger login-time handling later.
