...
The `RUN:` registry is built in `internal/menu/executor_runnables_registry.go`. Every registered command, the data it accepts, and the access it checks are documented in the [Menu Commands Reference](reference/menu-commands.md). A test in `internal/menu` fails when a registered command or login step has no row there.
